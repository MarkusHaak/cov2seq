...
import subprocess
import vcf
from .verify import mafft, variants_from_alignment, get_masked_bases, dataset_completion_test
...
def cov_from_sorted_bam(bam_fn):
    cov_fn = bam_fn + ".cov"
...
def get_nanopore_coverage(sample, artic_runs):
    ...
    fn = os.path.join(artic_dir, "{}.sorted.bam.cov".format(sample))
    if not os.path.exists(fn):
        cov_from_sorted_bam(fn[:-4])
    return np.genfromtxt(fn, delimiter='\t', usecols=(2), dtype=np.int32)

def get_nanopore_coverage_trimmed(sample, artic_runs):
    artic_dir = artic_runs.loc[sample, 'artic_dir']   
    fn = os.path.join(artic_dir, "{}.trimmed.rg.sorted.bam.cov".format(sample))
    if not os.path.exists(fn):
        cov_from_sorted_bam(fn[:-4])
    return np.genfromtxt(fn, delimiter='\t', usecols=(2), dtype=np.int32)
...
def get_nanopore_coverage_primertrimmed(sample, artic_runs):
    artic_dir = artic_runs.loc[sample, 'artic_dir']
    fn = os.path.join(artic_dir, "{}.primertrimmed.rg.sorted.bam.cov".format(sample))
    if not os.path.exists(fn):
        cov_from_sorted_bam(fn[:-4])
    return np.genfromtxt(fn, delimiter='\t', usecols=(2), dtype=np.int32)

#def get_nanopore_readgroup_coverage(sample, artic_runs, )
...
def create_sample_reports(args):
    ...
    #sample_schemes, primer_schemes = get_sample_primer_schemes(selected_samples, args.nanopore_dir)
    primers, amplicons = load_primer_schemes(args.primer_schemes_dir, primer_schemes)
    logger.info('Performing a dataset completion test for all selected samples.')
